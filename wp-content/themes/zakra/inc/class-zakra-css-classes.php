	...
	class Zakra_Css_Classes {
		...
		public function __construct() {
			...
			add_filter( 'zakra_footer_widget_container_class', array(
				$this,
				'zakra_add_footer_widget_cotainer_classes',
			) );
		...
		public function zakra_add_body_classes( $classes ) {
			...
			if ( is_singular( 'page' ) ) { // page.
				...
				if ( zakra_is_woocommerce_active() && ( is_checkout() || is_cart() || is_account_page() ) ) {
					$classes[] = get_theme_mod( 'zakra_structure_wc', 'tg-site-layout--default' );
				} elseif ( isset( $individual_layout ) && ! empty( $individual_layout ) && 'tg-site-layout--customizer' !== $individual_layout ) {
				...
				if ( zakra_is_woocommerce_active() && is_product() ) { // WC single product.
					$classes[] = get_theme_mod( 'zakra_structure_wc_product', 'tg-site-layout--default' );
				} elseif ( isset( $individual_layout ) && ! empty( $individual_layout ) && 'tg-site-layout--customizer' !== $individual_layout ) {
					$classes[] = $individual_layout;
				} else {
					$classes[] = get_theme_mod( 'zakra_structure_post', 'tg-site-layout--default' );
				}
			} elseif ( is_archive() || is_home() ) { // archive/ blog/ WC archive/ WC shop.
