    ...
    <ul class="entry-meta">
        <li>
            <a href="<?php the_permalink(); ?>" class="posted-on">
                <time datetime="<?php printf( __( '%1$s', 'elegant-pink' ), get_the_date('Y-m-d') ); ?>">
                    <?php printf( __( '%1$s', 'elegant-pink' ), get_the_date('j F Y') ); ?>
                </time>
        </a></li>
        ...
        <?php  
            if( is_single() && 'post' === get_post_type() ){
                ...
                if ( $categories_list && elegant_pink_categorized_blog() ) {
                    printf( '<li>' . esc_html__( '%1$s', 'elegant-pink' ) . '</li>', $categories_list );
                }
                
                $tags_list = get_the_tag_list( '', esc_html__( ', ', 'elegant-pink' ) );
                if ( $tags_list ) {
                    printf( '<li>' . esc_html__( '%1$s', 'elegant-pink' ) . '</li>', $tags_list );
                }
            }
        ?> 
    </ul>
...
function elegant_pink_category(){
    ...
    $categories_list = get_the_category_list( esc_html__( ', ', 'elegant-pink' ) );
    if ( $categories_list && elegant_pink_categorized_blog() ) {
        printf( '<div class="category">' . esc_html__( '%1$s', 'elegant-pink' ) . '</div>', $categories_list ); // WPCS: XSS OK.
    }
}
endif;
